...
with st.expander("🚀 **Getting Started - Quick Guide**", expanded=False):
    ...
    with col1:
        ...
        1. **Prepare your CSV** with these columns:
           ...
           - `portal - country` (client countries)
    ...
    with col2:
        ...
        ### 💡 Pro Tips
        - **Default view** shows all 2025 data (all sources & countries)
        - **Country breakdown**: Use "Show by Country" in Display Options
        - **Hover on charts** to see exact values
        - **Export data** in CSV, Excel, or JSON
        - **1/1/1970 dates** = No FTD yet (placeholders)
        """)
    
    st.info("📌 **Quick Start**: Just upload your CSV and the dashboard will automatically show your 2025 data. Use the left sidebar to filter by source, country, or date range.")
...
with st.expander("📚 **CSV Format Guide & Instructions**", expanded=False):
    ...
    with col1:
        ...
        st.markdown("#### 4️⃣ `portal - country` (Required)")
        st.markdown("""
        - **Purpose**: Client's country/location
    ...
    with col2:
        ...
        st.warning("""
        **⚠️ Important**: All required columns MUST be present in your CSV file:
        - `portal - ftd_time` (for FTD dates)
        - `DATE_CREATED` or `date_created` (for KYC dates)
        - `portal - source_marketing_campaign` (for sources)
        - `portal - country` (for country filtering)
        
        Column names are case-insensitive. All columns required even if using only one dashboard.
        """)
...
def load_df(file):
    ...
    actual_country_col = find_column_case_insensitive(df, country_col)
    ...
    country_col = actual_country_col
    ...
    # Fill missing sources and countries
    ...
    df.attrs['country_col'] = country_col
...
if uploaded is not None:
    ...
    try:
        ...
        - **Countries**: Column named `portal - country`
...
source_col = df.attrs.get('source_col', 'portal - source_marketing_campaign')
country_col = df.attrs.get('country_col', 'portal - country')
...
with st.sidebar:
    ...
    # Country selection - positioned between Source Selection and Date Range
    ...
    # Only show country selection for individual dashboards, not comparison
    if dashboard_type != "KYC & FTD Comparison":
        ...
        if "selected_countries" not in st.session_state:
            st.session_state.selected_countries = all_countries.copy()  # Select all countries by default
        ...
        st.markdown("---")
        country_container = st.container(height=300)  # Smaller height than sources to save space
        ...
        # For comparison dashboard, select all countries by default
        if "selected_countries" not in st.session_state or dashboard_type == "KYC & FTD Comparison":
            st.session_state.selected_countries = all_countries.copy()
        st.info("📊 Country selection disabled for comparison view - showing all countries")
    ...
    # Country breakdown option
    show_by_country = st.checkbox("Show by Country", value=False,
                                  help="Break down data by country instead of by source")
    
    if show_by_country:
        st.caption("🌍 **Country View**: Data grouped by client country")
        # Disable source grouping when showing by country
        if group_sources:
            st.warning("⚠️ Source grouping disabled when showing by country")
            group_sources = False
...
if dashboard_type == "KYC & FTD Comparison":
    ...
    mask_country = df[country_col].isin(selected_countries) if selected_countries else pd.Series(True, index=df.index)
...
# Initialize column reference for charts
...
if dashboard_type == "KYC & FTD Comparison":
    ...
    if len(months) > 0 and len(display_sources) > 0:
        ...
        full["clients"] = 0
        counts = full
elif show_by_country:
    # Group by country instead of source
    counts = (
    ...
    # Get unique countries from selected countries
...
if show_by_country:
    if len(selected_countries) < len(all_countries):
        if len(selected_countries) == 1:
            st.info(f"🌍 **Viewing data for 1 country:** {selected_countries[0]}")
        else:
            st.info(f"🌍 **Viewing data for {len(selected_countries)} selected countries.** Select more countries in the sidebar to see additional data.")
elif group_sources:
...
elif show_total and (len(display_sources) > 1 or group_sources or show_by_country):
...
chart_base = alt.Chart(chart_data).encode(
    ...
    color=alt.Color(f"{source_col_for_chart}:N", 
                   legend=alt.Legend(title="Country" if show_by_country else "Source"),
                   scale=color_scale),
    tooltip=[
        alt.Tooltip("ftd_month:T", title="Month", format="%B %Y"),
        alt.Tooltip(f"{source_col_for_chart}:N", title="Country" if show_by_country else "Source"),
...
if dashboard_type == "KYC & FTD Comparison":
    ...
    if not pivot.empty:
        ...
        with col1:
            ...
            view_suffix = "by_country" if show_by_country else ("grouped" if group_sources else "by_source")
            st.download_button(
                "📄 Download CSV", 
                data=csv_bytes, 
                file_name=f"ftd_{view_suffix}_{pd.Timestamp.now():%Y%m%d}.csv", 
                mime="text/csv",
                use_container_width=True
            )
        ...
        with col2:
            ...
            st.download_button(
                "📊 Download Excel",
                data=excel_bytes,
                file_name=f"ftd_analysis_{view_suffix}_{pd.Timestamp.now():%Y%m%d}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True
            )
        ...
        with col3:
            ...
            json_data = {
                "summary": {
                    ...
                    "view_mode": "by_country" if show_by_country else ("grouped_sources" if group_sources else "by_source"),
                    "countries_selected": len(selected_countries),
            ...
            st.download_button(
                "🔧 Download JSON",
                data=json_str,
                file_name=f"ftd_data_{view_suffix}_{pd.Timestamp.now():%Y%m%d}.json",
                mime="application/json",
                use_container_width=True
            )
    else:
